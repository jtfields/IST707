...
## Date: 1 Aug 2019
##################################################################
## This analysis will show methods for evaluating document similarity
## using word frequency from the Federalists Papers corpus created from
## the Library of Congress text files.
##
...
library(slam) #Compute the matrix cross-product
library(quanteda) #An R package for managing and analyzing text
library(SnowballC) #For collapsing words to a common root
library(proxy) #Efficient calculation of auto- and cross-proximities
library(stringr) #Functions designed to make working with strings as easy as possible.
library(textmineR) #An aid for text mining 
library(igraph) #Routines for simple graphs and network analysis. 
library(lsa) #Latent semantic analysis (LSA) like cosine
...
library(pracma) #Practical numerical math tools
library(cluster) #Bivariate Cluster Plot
...
FedCorpus <- Corpus(DirSource("FedCorpusAsNumbers"))
(FedCorpus)

## Create a list of filenames so they can be associated with Document Term Matrix (DTM)
FedFilesPath <- "/Users/johnfields/Library/Mobile Documents/com~apple~CloudDocs/Syracuse/IST707/Homework/Week 4/FedCorpusAsNumbers"
...
(Feddataframe <- data.frame(text=sapply(CleanFedCorpus, identity), 
                          stringsAsFactors=F))
write.csv(Feddataframe, "CorpusFEDoutput.csv")
...
write.csv(FeddataframeLemm, "CorpusFEDoutputLemm.csv")
...
wordcloud(words = names(word.freq), freq = word.freq*2, min.freq = 100,random.order = F,colors=brewer.pal(3, "Set1"))
...
## Use Agglomeritive Hierarchical Clustering to cluster documents and visualize normalized DTM
## The dendrogram: terms higher in the plot appear more frequently within the corpus
...
rect.hclust(fit3, k = 2) # cut tree into 2 clusters 
...
## Use 1 - cosine sim to get the nearness - 1 is nearest.
...
fviz_nbclust(ClusterM, kmeans, method = "silhouette") +
  geom_vline(xintercept = 4, linetype = 2)+
  labs(subtitle = "Silhouette")

# Determine the optimum number of clusters
fviz_nbclust(ClusterM, kmeans, method = "wss") +
  geom_vline(xintercept = 4, linetype = 2)+
